    ...
    def read_txt(fname):
        if "://" in fname:
            ...
            with open(fname, "r") as file:
                value = file.read()
        return value
    # Special handling for Hugging Face datasets, including HF_ENDPOINT mirrors.
    # Git LFS files have a different file path to access the raw data than other files
    hf_endpoint = os.environ.get("HF_ENDPOINT", "https://huggingface.co").rstrip("/")
    hf_dataset_prefixes = (
        "https://huggingface.co/datasets",
        f"{hf_endpoint}/datasets",
    )
    if data_dir.startswith(hf_dataset_prefixes):
        # Format: <HF_ENDPOINT>/datasets/<USERNAME>/<REPO>/tree/<BRANCH>
