#!/bin/bash
# Reconstruction with the continuous Cosmos tokenizer CosmosCV8x16x16.
# Usage:
#   CUDA_VISIBLE_DEVICES=0 SHORT_SIZES="256" bash cosmos_cv8x16x16.sh
set -e
...
MODEL="${MODEL:-CosmosCV8x16x16}"
MODEL_NAME="${MODEL_NAME:-$MODEL}"
COSMOS_PATH="${COSMOS_PATH:-$SCRIPT_DIR/Cosmos-Tokenizer}"
MODEL_ROOT="${MODEL_ROOT:-$MODEL_ZOO}"
...
MODEL_DIR="$MODEL_ROOT/Cosmos-0.1-Tokenizer-CV8x16x16"

if [ ! -f "$COSMOS_PATH/cosmos_tokenizer/video_lib.py" ]; then
    echo "Missing Cosmos-Tokenizer code: $COSMOS_PATH"
    echo "See ../reconstruct.md for download instructions."
    exit 1
fi

if [ ! -f "$MODEL_DIR/encoder.jit" ] || [ ! -f "$MODEL_DIR/decoder.jit" ]; then
    echo "Missing Cosmos checkpoint files in: $MODEL_DIR"
    echo "See ../reconstruct.md for download instructions."
    exit 1
fi
...
for DATA in "${TEXT_DATAS[@]}"; do
    for SHORT_SIZE in "${SHORT_SIZES[@]}"; do
        ...
        for IDX in $(seq 0 $((CHUNKS - 1))); do
            GPU_IDX=$((IDX % ${#GPULIST[@]}))
            CUDA_VISIBLE_DEVICES=${GPULIST[$GPU_IDX]} python cosmos_rec.py \
