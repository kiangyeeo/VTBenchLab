#!/bin/bash
# Reconstruction with the Step-Video 8x16x16 VAE.
# Usage:
#   CUDA_VISIBLE_DEVICES=0 SHORT_SIZES="256" bash stepvideo_8x16x16.sh
set -e
...
MODEL_NAME="${MODEL_NAME:-stepvideo_8x16x16}"
STEPVIDEO_PATH="${STEPVIDEO_PATH:-$SCRIPT_DIR/Step-Video-T2V}"
MODEL_PATH="${MODEL_PATH:-$MODEL_ZOO/stepvideo-t2v/vae/vae_v2.safetensors}"
...
if [ ! -f "$STEPVIDEO_PATH/stepvideo/vae/vae.py" ]; then
    echo "Missing Step-Video-T2V code: $STEPVIDEO_PATH"
    echo "See ../tokenizer_setup_9x.md for download instructions."
    exit 1
fi

if [ ! -f "$MODEL_PATH" ]; then
    echo "Missing Step-Video VAE checkpoint: $MODEL_PATH"
    echo "See ../tokenizer_setup_9x.md for download instructions."
    exit 1
fi
...
for DATA in "${TEXT_DATAS[@]}"; do
    for SHORT_SIZE in "${SHORT_SIZES[@]}"; do
        ...
        for IDX in $(seq 0 $((CHUNKS - 1))); do
            GPU_IDX=$((IDX % ${#GPULIST[@]}))
            CUDA_VISIBLE_DEVICES=${GPULIST[$GPU_IDX]} python stepvideo_rec.py \
                --video_path "$DATA_ROOT/videos/text_data/$DATA" \
                --save_path "$RECON_ROOT/$MODEL_NAME/text_data/$DATA" \
                --stepvideo_path "$STEPVIDEO_PATH" \
