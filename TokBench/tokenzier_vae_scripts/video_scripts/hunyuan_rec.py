...
def main(args):
    ...
    for chunk in tqdm(chunk_inputs):
        ...
        video_length = len(allframes)

        num_video_chunks = math.ceil(len(allframes)/241)
        all_output_frames = []
        while num_video_chunks>0:
            num_video_chunks -=1
            frames = allframes[:241]
            allframes = allframes[241:]


            ori_length = len(frames)
            padding_length = math.ceil((ori_length-1)/4) * 4 +1
            num_pad_frame = padding_length-ori_length
            padded_frames = [frames[-1]]*num_pad_frame
            frames.extend(padded_frames)
            # print(ori_length, num_pad_frame)
            # frames = frames[:22]  # 5 9 13 17  4n+1
            ...
            all_output_frames.extend(output_frames)
        assert len(allframes)==0

        assert len(all_output_frames) == video_length

        reconframes = np.stack(all_output_frames,axis=0)
        writer = imageio.get_writer('{}/{}'.format(image_save_pth,chunk[0]), fps=video_fps)
        for frame in reconframes:
