#!/bin/bash
# Reconstruction with the TokenFlow (clipb 32k enhanced) image tokenizer.
# Usage:
#   CUDA_VISIBLE_DEVICES=0 PADDING_SIZES="256" bash tokenflow.sh
set -e
...
MODEL_NAME="${MODEL_NAME:-tokenflow}"
TOKENFLOW_PATH="${TOKENFLOW_PATH:-$SCRIPT_DIR/TokenFlow}"
CKPT_PATH="${CKPT_PATH:-$MODEL_ZOO/TokenFlow/tokenflow_clipb_32k_enhanced.pt}"
...
require_dir "$TOKENFLOW_PATH" "TokenFlow code directory"
require_file "$CKPT_PATH" "TokenFlow checkpoint"
...
for DATA in "${TEXT_DATAS[@]}"; do
    for PADDING_SIZE in "${PADDING_SIZES[@]}"; do
        ...
        for IDX in $(seq 0 $((CHUNKS - 1))); do
            GPU_IDX=$((IDX % ${#GPULIST[@]}))
            CUDA_VISIBLE_DEVICES=${GPULIST[$GPU_IDX]} python tokenflow_rec.py \
                --image_path "$DATA_ROOT/images/text_data/$DATA" \
                --save_path "$RECON_ROOT/$MODEL_NAME/text_data/$DATA" \
                --tokenflow_path "$TOKENFLOW_PATH" \
                --ckpt_path "$CKPT_PATH" \
