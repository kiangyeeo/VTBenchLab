#!/bin/bash
# Reconstruction with VA-VAE f16d32.
# Usage:
#   CUDA_VISIBLE_DEVICES=0 PADDING_SIZES="256" bash vavae_f16d32.sh
set -e
...
MODEL_NAME="${MODEL_NAME:-vavae_f16d32}"
LIGHTNINGDIT_PATH="${LIGHTNINGDIT_PATH:-$SCRIPT_DIR/LightningDiT}"
CKPT_PATH="${CKPT_PATH:-$MODEL_ZOO/vavae/vavae-imagenet256-f16d32-dinov2.pt}"
...
require_dir "$LIGHTNINGDIT_PATH/tokenizer" "LightningDiT tokenizer directory"
require_file "$CKPT_PATH" "VA-VAE checkpoint"
...
for DATA in "${TEXT_DATAS[@]}"; do
    for PADDING_SIZE in "${PADDING_SIZES[@]}"; do
        ...
        for IDX in $(seq 0 $((CHUNKS - 1))); do
            GPU_IDX=$((IDX % ${#GPULIST[@]}))
            CUDA_VISIBLE_DEVICES=${GPULIST[$GPU_IDX]} python vavae_rec.py \
                --image_path "$DATA_ROOT/images/text_data/$DATA" \
                --save_path "$RECON_ROOT/$MODEL_NAME/text_data/$DATA" \
                --lightningdit_path "$LIGHTNINGDIT_PATH" \
                --ckpt_path "$CKPT_PATH" \
