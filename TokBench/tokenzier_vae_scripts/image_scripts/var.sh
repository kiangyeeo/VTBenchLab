#!/bin/bash
# Reconstruction with the VAR VAE (ch160 v4096 z32) image tokenizer.
# Usage:
#   CUDA_VISIBLE_DEVICES=0 PADDING_SIZES="256" bash var.sh
set -e
...
MODEL_NAME="${MODEL_NAME:-var}"
VAR_PATH="${VAR_PATH:-$SCRIPT_DIR/VAR}"
CKPT_PATH="${CKPT_PATH:-$MODEL_ZOO/var/vae_ch160v4096z32.pth}"
...
require_dir "$VAR_PATH" "VAR code directory"
require_file "$CKPT_PATH" "VAR VAE checkpoint"
...
for DATA in "${TEXT_DATAS[@]}"; do
    for PADDING_SIZE in "${PADDING_SIZES[@]}"; do
        ...
        for IDX in $(seq 0 $((CHUNKS - 1))); do
            GPU_IDX=$((IDX % ${#GPULIST[@]}))
            CUDA_VISIBLE_DEVICES=${GPULIST[$GPU_IDX]} python var_vae_rec.py \
