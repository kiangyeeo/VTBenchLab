#!/bin/bash
# Reconstruction with the taming-transformers VQGAN (ImageNet f16, codebook 16384).
# Usage:
#   CUDA_VISIBLE_DEVICES=0 PADDING_SIZES="256" bash vqgan.sh
set -e
...
MODEL_NAME="${MODEL_NAME:-vqgan}"
TAMING_PATH="${TAMING_PATH:-$SCRIPT_DIR/taming-transformers}"
VQGAN_DIR="${VQGAN_DIR:-$MODEL_ZOO/taming_vqgan_imagenet_f16_16384}"
CONFIG_PATH="${CONFIG_PATH:-$VQGAN_DIR/model.yaml}"
CKPT_PATH="${CKPT_PATH:-$VQGAN_DIR/last.ckpt}"
...
require_dir "$TAMING_PATH" "taming-transformers code directory"
require_file "$CONFIG_PATH" "Taming VQGAN config"
require_file "$CKPT_PATH" "Taming VQGAN checkpoint"
