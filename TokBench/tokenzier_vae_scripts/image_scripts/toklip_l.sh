#!/bin/bash
# Reconstruction with the TokLIP-L VQ image tokenizer.
# Usage:
#   CUDA_VISIBLE_DEVICES=0 PADDING_SIZES="256" bash toklip_l.sh
set -e
...
MODEL_NAME="${MODEL_NAME:-toklip_l}"
...
require_file "$TOKLIP_CKPT_PATH" "TokLIP-L checkpoint"
...
for DATA in "${TEXT_DATAS[@]}"; do
    for PADDING_SIZE in "${PADDING_SIZES[@]}"; do
        ...
        for IDX in $(seq 0 $((CHUNKS - 1))); do
            GPU_IDX=$((IDX % ${#GPULIST[@]}))
            CUDA_VISIBLE_DEVICES=${GPULIST[$GPU_IDX]} python toklip_l_rec.py \
