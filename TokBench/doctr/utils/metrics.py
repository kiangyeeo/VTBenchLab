...
def string_match_with_ned(word1: str, word2: str):
    """Calculates the string match metric
    ...
    raw_ned = edit_distance(word1, word2) / max(len(word1), len(word2))
    caseless_ned = edit_distance(word1.lower(), word2.lower()) / max(len(word1), len(word2))
    unicase_ned = edit_distance(anyascii(word1).lower(), anyascii(word2).lower()) / max(len(word1), len(word2))
    label_length = len(word1)

    return raw_match, caseless_match, anyascii_match, unicase_match, raw_ned, caseless_ned, unicase_ned, label_length
...
class RecMetric:
    ...
    def update(
        ...
        for gt_word, pred_word in zip(gt, pred):
            _raw, _caseless, _anyascii, _unicase, _raw_ned, _caseless_ned, _unicase_ned, _label_length = string_match_with_ned(gt_word, pred_word)
            self.raw += int(_raw)
            self.caseless += int(_caseless)
            self.anyascii += int(_anyascii)
            self.unicase += int(_unicase)
            self.raw_ned += _raw_ned
            self.caseless_ned += _caseless_ned
            self.unicase_ned += _unicase_ned
            self.label_length += _label_length
    ...
    def summary(self) -> dict[str, float]:
        ...
        return dict(
            ...
            anyascii=self.anyascii / self.total,
            unicase=self.unicase / self.total,
            raw_ned=1-self.raw_ned / self.total,
            caseless_ned=1-self.caseless_ned / self.total,
            unicase_ned=1-self.unicase_ned / self.total,
            label_length =self.label_length / self.total
        )
...
class RecMetricWithDetails:
    ...
    def update(
        ...
        for gt_word, pred_word in zip(gt, pred):
            _raw, _caseless, _anyascii, _unicase, _raw_ned, _caseless_ned, _unicase_ned, _label_length = string_match_with_ned(gt_word, pred_word)
            self.raw.append(int(_raw))
            self.caseless.append(int(_caseless))
            self.anyascii.append(int(_anyascii))
            self.unicase.append(int(_unicase))
            self.raw_ned.append(_raw_ned)
            self.caseless_ned.append(_caseless_ned)
            self.unicase_ned.append(_unicase_ned)
            self.label_length.append(_label_length)
            self.results.append(dict(gt=gt_word, pred=pred_word))
    ...
    def summary(self) -> dict[str, list[Any]]:
        ...
        return dict(
            raw=sum(self.raw) / self.total,
            caseless=sum(self.caseless) / self.total,
            anyascii=sum(self.anyascii) / self.total,
            unicase=sum(self.unicase) / self.total,
            raw_ned=1 - sum(self.raw_ned) / self.total,
            caseless_ned=1 - sum(self.caseless_ned) / self.total,
            unicase_ned=1 - sum(self.unicase_ned) / self.total,
            label_length=sum(self.label_length) / self.total
        )
    ...
    def detail(self):
        ...
        return dict(
            ...
            anyascii=self.anyascii,
            unicase=self.unicase,
            raw_ned=self.raw_ned,
            caseless_ned=self.caseless_ned,
            unicase_ned=self.unicase_ned,
