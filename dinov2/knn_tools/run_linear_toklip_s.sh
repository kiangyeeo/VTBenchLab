#!/usr/bin/env bash
# TokLIP-S VQ z_q linear-probing evaluation on ImageNet-1k.
set -euo pipefail

REPO="${REPO:-/cache/ma-user/VTBenchLab/dinov2}"
DATA="${DATA:-/cache/ma-user/VTBenchLab/data/imagenet1k}"
EXTRA="${EXTRA:-$DATA/extra}"
OUTDIR="${OUTDIR:-/cache/ma-user/VTBenchLab/outputs/vae_linear_probing/toklip_s_zq_256}"
...
echo ">> TokLIP-S z_q linear probing"
echo "   out=$OUTDIR"
echo "   ckpt=$CKPT_PATH"
