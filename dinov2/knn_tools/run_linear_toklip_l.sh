...
LEGACY_OUTDIR="/cache/ma-user/VTBenchLab/outputs/vae_linear_probing/toklip_l_semantic_384"
...
setup_linear_dataset_args "toklipl" "$LEGACY_OUTDIR" "$@"
setup_linear_training_args "$@"

[ -d "$TOKLIP_PATH" ] || { echo "!! missing TokLIP code dir: $TOKLIP_PATH"; exit 1; }
[ -f "$CKPT_PATH" ] || { echo "!! missing TokLIP-L checkpoint: $CKPT_PATH"; exit 1; }
